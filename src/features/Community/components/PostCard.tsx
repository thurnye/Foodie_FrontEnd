...
import { votePost, removePostVote, reactToPost, removePostReaction, deletePost } from '../redux/community.thunk';
    ...
    <Card sx={{ mb: 2, borderRadius: 2 }}>
      <CardContent>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2 }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              ...
              {post.isPinned && (
                <Chip label="Pinned" size="small" color="primary" sx={{ ml: 1, height: 18 }} />
              )}
            </Typography>
          </Box>
          ...
          <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
            {user?.id === (typeof post.author === 'string' ? post.author : post.author._id) && (
        ...
        {post.media && post.media.length > 0 && (
          <Box sx={{ mb: 2 }}>
            {post.media.map((media, index) => (
              <Box key={index} sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', mb: 1 }}>
                {media.type === 'image' ? (
                  <img
                    src={media.url}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mt: 2, gap: 1 }}>
          {/* Voting */}
          <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: '#f5f5f5', borderRadius: 10, px: 1 }}>
            <IconButton
              size="small"
          ...
          <Button
            size="small"
            startIcon={userReaction ? reactionIcons[userReaction.type] : <ThumbUpAltOutlined />}
