...
const EventBasicInfo = forwardRef<BasicInfoStepRef, Props>(
  ({ data, onChange }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      validate: () => {
        if (
          ...
          return {
            valid: false,
            error: 'Please fill in all required fields in Basic Info.',
          };
        }
        ...
        if (
          parseInt(data.capacity, 10) <= 0 ||
          Number.isNaN(parseInt(data.capacity, 10))
        ) {
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        >
          <Typography variant='body2' gutterBottom sx={{}}>About Event</Typography>
          <TextEditor
            getContents={(htmlValue) => update('description', htmlValue)}
