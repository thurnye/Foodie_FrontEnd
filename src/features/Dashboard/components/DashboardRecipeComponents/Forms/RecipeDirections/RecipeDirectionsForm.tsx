...
import { IIngredient, IMethod, IRecipeDirections } from '../../../../../Recipe/types/recipe.types';


interface RecipeDirectionsFormProps {
  setData: (data: IRecipeDirections) => void;
  defaultValues: IRecipeDirections;
}

interface ErrorState {
  error: boolean;
  type: 'method' | 'ingredients' | '';
  message: string;
}

// ---- Component ---- //
const RecipeDirectionsForm: React.FC<RecipeDirectionsFormProps> = ({
  setData,
  defaultValues,
}) => {
  ...
  const [methods, setMethods] = useState<IMethod[]>(defaultValues.methods);
  ...
  const onSubmit = () => {
    ...
    setIsError({ error: false, type: '', message: '' });
    const directions: IRecipeDirections = { methods, ingredients };
    setData(directions);
  };
    ...
    <Container>
      ...
      <Box sx={{ mb: 2 }}>
        {activeSection !== 'methodsForm' && (
          <Box onClick={() => methods?.length === 0 && setActiveSection('methodsForm')}>
            <Card
      ...
      <Box sx={{ mb: 2 }}>
        {activeSection !== 'faqForm' && (
          <Box onClick={() => ingredients?.length === 0 && setActiveSection('faqForm')}>
            <Card
