...
import { getAllDatesInRange, getTimeZone, filterSortSchedule, getLocalDateString, findMinimumPriceOrFreeTicket } from '../../../../util/commons';
...
const AllEvents = () => {
  ...
  const getEventStartDate = (schedule) => {
    ...
    const allDateRange = [];
      ...
      >
        {`${getLocalDateString(filterSortSchedule(schedule)[0].start)} ${getTimeZone()}`}
      </Typography> 
    }
  ...
  const handleEventClick = (event) => {
    ...
    const formattedEventTitle = event.basicInfo.eventTitle.toLocaleLowerCase().replaceAll(" ", "-");
      ...
      <Grid container  columns={{ xs: 4, sm: 8, md: 12 }} sx={{
        ...
        {events.map((event, index) => {
          ...
          <Grid item xs={4} sm={4} md={4} key={index} sx={{
            ...
            {/* <Link to={'event'}> */}
              <Card sx={{ maxWidth: 345}} onClick={()=> handleEventClick(event)}>
                <Box sx={{position: 'relative'}}>
                  ...
                  <Box sx={{
                    ...
                    <CardActions disableSpacing>
                      <IconButton aria-label="add to favorites" onClick={() => handleFavs(_id)}>
                        {favs.includes(_id) ? 
                        <FavoriteIcon sx={{
                          height: 35,
