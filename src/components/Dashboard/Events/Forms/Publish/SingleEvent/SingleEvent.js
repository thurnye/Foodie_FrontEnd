...
import ImageLayout from '../../Details/ImageLayout'
...
const SingleEvent = ({isLoaded,isPreview, data}) => {
  ...
  const isExpired = () => {
    ...
    const startDate = basicInfo.dateOccurrence === 'single' ? basicInfo.dateTime.start : sortedSchedule[0].start;
    const endDate = basicInfo.dateOccurrence === 'single' ? basicInfo.dateTime.end : sortedSchedule[sortedSchedule.length -1 ].end;
    const activeSections = [];

    const updateSections = sections.map((el) => {
      el.ticketTypes.forEach((ticket) => {
        const {salesEnd, period, periodFrame} = ticket;
        
        if(periodFrame === 'Before event starts'){
          ticket.stopSales =  backDatedDate(startDate, salesEnd, period)
        }
        if(periodFrame === 'Before event ends'){
          ticket.stopSales =  backDatedDate(endDate, salesEnd, period)
        }
      })
      return el;
    })
    ...
    updateSections.forEach((el) => {
      // add ticket with available capacity
  ...
  useEffect(() => setActiveTickets(isExpired()), [tickets]);
