...
import {
  ...
  CheckBoxField,
  TimeInput,
} from '../../../../Forms/FormContainer/FormContainer';
...
const BasicForm = ({ isLoaded, setData, defaultValues }) => {
  ...
  const [dateOccurrence, setDateOccurrence] = React.useState(
    defaultValues.dateOccurrence
  );
  const [proceed, setProceed] = useState(false);
  const [selected, setSelected] = useState(
    defaultValues.dateTime.start ? true : false
  );
          ...
          <div>
            ...
            <Box>
              <Stack spacing={2} direction='row'>
                {['Venue', 'Online event', 'To be announced'].map((el) => (
                  <Button
                    sx={{}}
                    variant={
                      locationValue === el.toLowerCase()
                        ? 'contained'
                        : 'outlined'
                    }
                    onClick={() => {
                ...
                {locationValue === 'online' && (
                  <>
                    ...
                    >
                      ...
                      links to live streams and more
                    </Typography>
                  </>
                )}
              </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <Typography
              ...
              gutterBottom
            >
              Date and time Tell event-goers when your event starts and ends so
                ...
                {dateOccurrence === 'single' && (
                  <>
                    <Typography sx={{ fontSize: 14 }} color='text.secondary'>
                      Single event happens once and can last multiple days with
                      a fix time.
                    </Typography>
                    ...
                    <Box>
                      ...
                      <Box>
                        {selected &&
                          Object.entries(dateState).map(([key, value]) => (
                            <Box key={key}>
                              <Typography
                                sx={{ fontSize: 14, mb: 2, fontWeight: 700 }}
                              >
                                {key}
                              </Typography>
                              <Box
                              ...
                              >
                                ...
                                <TimeInput
                                  ...
                                  label={`${key} Time`}
                                  isRequired={
                                    dateOccurrence === 'single' ? true : false
                                  }
                                />
                              </Box>
                            </Box>
                ...
                {dateOccurrence === 'recurring' && (
                  <>
                    <Typography
                      ...
                      color='text.secondary'
                    >
                      You’ll be able to set a schedule for your recurring event
                      in the next step. Event details and ticket types will
